"""The município list — local, committed, instant (decision D12).

The *list* is a CSV in ``data/``; the *geometry* comes from an Earth Engine asset
(``config/datasets.py::IBGE_MUNICIPIOS``). ``cod_municipio_ibge`` joins them.

Deriving the list from distinct ``cod_municipio_ibge`` in a CAR UF layer would
mean scanning hundreds of thousands of rows to populate a dropdown — exactly what
constraint C5 exists to prevent (doc/11-search-and-navigation.md §4).
"""
...
def for_uf(uf: str) -> List[Dict[str, Any]]:
    """Every município in a UF, alphabetically — the cascading selector."""
...
def search(text: str, *, uf: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    rows = for_uf(uf) if uf else _load()
...
# --------------------------------------------------------------------------- #
# Geometry — from Earth Engine (decision D12)
# --------------------------------------------------------------------------- #
_geom_cache: Dict[int, Dict[str, Any]] = {}


def geometry(cod_municipio_ibge: int) -> Optional[Dict[str, Any]]:
    """The município boundary as GeoJSON, for framing and clipping.
...
__all__ = ["for_uf", "by_code", "search", "normalise", "geometry", "bounds"]
