...
committed and instant for the same reason the município list is (decision D12,
constraint C5): a type-ahead must not wait on a round trip, and framing the map
on a hit must not wait on a polygon. The bbox in each row is computed from the
...
browser by an ordinary HTTP GET (``camposcope/api``) rather than pushed through
the WebSocket, exactly as :mod:`camposcope.services.biomes` explains for the
...
# Layer specs
# --------------------------------------------------------------------------- #
#: Paths served by the backend (see :mod:`camposcope.api`). Relative — the map
