...
Canada's counterpart to :mod:`camposcope.config.mapbiomas`. The legend and the
colour map are the published AAFC ones, ported via Naturametrics' own Canada
page; the Portuguese labels exist because this app is bilingual and the ACI
legend is only distributed in English and French.

**Two coverage facts shape this whole page, and both are properties of the
product rather than of this code — verified against the asset on 2026-08-27:**

1. **Years.** ``AAFC/ACI`` holds 2009–2025. 2009 and 2010 cover the Prairies
   only (AB/SK/MB); national coverage starts in **2011**. A parcel in coastal
   BC therefore has no 2009 or 2010 column, and that is data, not a bug.
2. **Northern limit.** The inventory stops at roughly **54–58°N**, ragged and
   ...
   cover. In British Columbia that puts the Peace River country inside it and
   most of the northwest outside.

Consequence for the UI, and it is the opposite of the Brazil page's rule: a
parcel north of the ACI limit is **still analysable**. The parcel itself, its
rings, Hansen forest change, ESA CCI biomass, MODIS fire and NTEMS VLCE2 all
answer up there — only the Cobertura and Transições tabs have nothing to show,
and they say so in their own panel rather than blocking the property. That is
why ``canada/services/aci_history.py`` returns an empty frame rather than raising.

Where MapBiomas ships forty years as forty bands of one image, the ACI is an
ImageCollection of one single-band image per year. ``canada/services/aci_history.py``
stacks it into the same one-image-many-bands shape first, which is what keeps
the whole "17 years × N zones" matrix a single ``reduceRegions`` round trip.
"""
...
def band_for_year(year: int) -> str:
    """Band name after stacking — see ``canada/services/aci_history.py``.
...
# Class groups
# --------------------------------------------------------------------------- #
# Unlike the Brazil page's MapBiomas groups (still provisional — D6 in
# doc/10-open-decisions.md), these come
...
#: (``components/charts.py::_STACK_PRIORITY``):
...
# --------------------------------------------------------------------------- #
# Group aliases — the names the shared analysis code asks for
# --------------------------------------------------------------------------- #
# ``canada/services/connectivity.py`` and ``canada/components/charts.py`` are
# ports of modules that name their class groups the MapBiomas way. Rather than
# rename the concept in every ported call site (and lose the diff against the
# Brazil original), the ACI's own legend-derived groups are aliased to those
# names here. These are aliases, not new groupings: the sets above are still the
# single definition.

#: Closed-canopy forest — what "a forest fragment" means for the connectivity
#: metric. Coniferous, broadleaf, mixedwood and the undifferentiated class.
FOREST_FORMATIONS = FOREST

#: The ACI has no savanna analogue. Kept as an empty set rather than omitted so
#: ported code that unions it needs no conditional.
SAVANNA_FORMATIONS = frozenset()

#: Natural, non-forest: wetland, peatland, shrubland, grassland.
NATURAL_NON_FOREST = WETLAND | GRASS_SHRUB

#: The ACI does not distinguish planted forest from natural stands — silviculture
#: is simply classed as forest. Recorded as empty rather than guessed, because
#: guessing would put plantations into "natural vegetation" invisibly.
PLANTED_FOREST = frozenset()

#: ``10`` is "Cloud", which is genuinely absence of observation.
NO_DATA_CLASSES = NO_DATA
