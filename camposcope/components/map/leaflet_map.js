// Persistent Leaflet map for Camposcope.
//
...
function useCamposcopeMap(containerRef, config, layers, overlays, vectors, onMapClick,
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      incoming.forEach((spec, index) => {
        ...
        // A WMS layer's identity is its URL *and* its `layers`/`cql_filter`:
        // switching UF keeps the same endpoint but must replace the layer.
        const sameSource = existing && existing.url === spec.url &&
          existing.wmsLayers === (spec.layers || null) &&
          existing.cqlFilter === (spec.cql_filter || null);
        ...
        // Two kinds of tile source. XYZ templates (Earth Engine, basemaps)
        // are the common case; `kind: "wms"` exists for the CAR property
        // boundaries, which are served by geoserver.car.gov.br as WMS and
        // cannot be expressed as an XYZ template — Leaflet has to compute the
        // bbox per tile. See doc/05-sicar-geoserver.md §5.5.
        const layer = spec.kind === "wms"
          ? L.tileLayer.wms(spec.url, {
              layers: spec.layers,
              format: spec.format || "image/png",
              transparent: spec.transparent !== false,
              version: spec.version || "1.1.1",
              opacity: spec.opacity != null ? spec.opacity : 1.0,
              attribution: spec.attribution || "",
              maxZoom: 22,
              zIndex: zIndex,
              // Drawing 218 000 polygons is the server's problem, not ours;
              // but asking for them below z8 produces an unreadable smear and
              // a pointless request, so the caller sets minZoom.
              minZoom: spec.min_zoom != null ? spec.min_zoom : 0,
              ...(spec.cql_filter ? { CQL_FILTER: spec.cql_filter } : {}),
            })
        ...
        registry.set(spec.id, {
          layer: layer,
          url: spec.url,
          wmsLayers: spec.layers || null,
          cqlFilter: spec.cql_filter || null,
  ...
  const applyClips = () => {
    ...
    const handle = map.getContainer().querySelector(".cs-swipe-handle");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let handle = container.querySelector(".cs-swipe-handle");
      ...
      handle = L.DomUtil.create("div", "cs-swipe-handle", container);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Ring order = order of appearance in the collection, which
      // services/zones.py emits innermost-first. Computed once here rather than
      // per feature inside style().
      const RING_ORDER = ((overlays && overlays.features) || [])
        .filter((f) => (f.properties || {}).role === "ring")
        .map((f) => (f.properties || {}).zone_key);
      ...
      const group = L.geoJSON(overlays, {
        ...
        style: (feature) => {
          ...
          // Camposcope's two zone roles (doc/02-architecture.md §3). The
          // property is the only saturated thing on the map — it is the object
          ...
          if (props.role === "ring") {
            // Outline only: a filled ring would hide the land-cover layer
            // underneath, which is the one thing these rings exist to frame.
            const ringColors = ["#8ec5ff", "#6aa9e9", "#4a86c4", "#33639b"];
