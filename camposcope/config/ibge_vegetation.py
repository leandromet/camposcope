...
**Ported from Naturametrics** (``naturametrics/config/ibge_vegetation.py``) —
the asset, taxonomy and class ids are facts about the dataset, not about which
app is asking. Used here for the SPOT/IBGE comparison tab (doc/03-roadmap.md
Phase 5 addendum), not for buffer histories. The colour palette below is
generated fresh by this module rather than kept in lockstep with
Naturametrics' own copy — see :data:`IBGE_VEG_COLOR_MAP`'s docstring for why
neither app ports IBGE's literal official colours any more.
...
style (``simbologia_vege_area/vege_legenda2.qml``); the colours are not.
