...
``scripts/fetch_uf_boundaries.py`` and ``scripts/fetch_municipios.py``.
...
``data/terras_indigenas.geojson.gz`` and ``data/unidades_conservacao.geojson.gz``
    ...
    ``camposcope/api`` serves them (services/territorios.py never
    re-compresses).
...
runtime dependency of this app — deliberately (doc/09-dev-environment.md §5).
...
``shapely`` (both already here) read it directly. Committing the outputs is the
same call ``data/uf_boundaries.simplified.geojson`` already makes: small,
static, derived, and a deploy cannot rebuild it because the source
GeoPackages are not in this repo.
