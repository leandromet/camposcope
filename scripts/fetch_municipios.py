...
Decision **D12** ([doc/10](../doc/10-open-decisions.md)): the município *list* is a committed
local table, not a query. Deriving it from distinct ``cod_municipio_ibge`` in a CAR UF layer
would mean scanning hundreds of thousands of rows to populate a dropdown — exactly what
constraint C5 exists to prevent. The Earth Engine município asset supplies *geometry*;
